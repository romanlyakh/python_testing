...
def app(request):
    ...
    web_config = load_cofig(request.config.getoption("--target"))['web']
...
def db(request):
    db_config = load_cofig(request.config.getoption("--target"))['db']
...
@pytest.fixture(scope="session")
def orm(request):
  orm_config = load_cofig(request.config.getoption("--target"))['db']
